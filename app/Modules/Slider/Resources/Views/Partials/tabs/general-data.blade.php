...
<div class="tab-content mt-5" id="myTabContent">
    ...
    <div class="tab-pane fade {{$key == 0 ? 'active show' : '' }}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
        <div class="form-group  p-0 m-0 mb-5 pt-5">
            ...
            <input type="text" class="form-control col-md-8" name="title_{{$lang['prefix']}}" value="{{ isset($model) ? $model->{'title_'.$lang['prefix']} : old('title_'.$lang['prefix'])}}" placeholder="{{ trans('Section::section.form.title_'.$lang['prefix']) }}">
        </div>
        <div class="form-group  p-0 m-0 mb-5 pt-5">
            <label for="inputEmail3">{{ trans('Section::section.form.description_'.$lang['prefix']) }} :</label>                        
            <textarea  class="form-control col-md-8" name="description_{{$lang['prefix']}}" placeholder="{{ trans('Section::section.form.description_'.$lang['prefix']) }}" cols="30" rows="10">{{ isset($model) ? $model->{'description_'.$lang['prefix']} : old('title_'.$lang['prefix'])}}</textarea>
        </div>

        <div class="form-group  p-0 m-0 mb-5 pt-5">
            <label for="inputEmail3">{{ trans('Slider::slider.form.extra_heading') }} :</label>                        
            <input type="text" class="form-control col-md-8" name="metadata[extra_heading][{{ $lang['prefix'] }}]" value="{{ isset($model) ? $model->metadata['extra_heading'][$lang['prefix']] : old('metadata[extra_heading]['.$lang['prefix'].']')}}" placeholder="{{ trans('Slider::slider.form.extra_heading') }}">
        </div>

        <div class="form-group  p-0 m-0 mb-5 pt-5">
            <label for="inputEmail3">{{ trans('Slider::slider.form.button_text') }} :</label>                        
            <input type="text" class="form-control col-md-8" name="metadata[button_text][{{ $lang['prefix'] }}]" value="{{ isset($model) ? $model->metadata['button_text'][$lang['prefix']] : old('metadata[button_text]['.$lang['prefix'].']')}}" placeholder="{{ trans('Slider::slider.form.button_text') }}">
        </div>
    </div>
    @endforeach
</div>
...
<div class="form-group  p-0 m-0 mb-5 pt-5">
    <label for="inputEmail3">{{ trans('Slider::slider.form.button_link') }} :</label>                        
    <input type="text" class="form-control col-md-8" name="metadata[button_link]" value="{{ isset($model) ? $model->metadata['button_link'] : old('metadata[button_link]')}}" dir="ltr" placeholder="{{ trans('Slider::slider.form.button_link') }}">
</div>

<div class="form-group  p-0 m-0 mb-5">
    <label class="">{{ trans('Section::section.form.page') }} :</label>
    <select name="page_id" class="form-control col-md-8" data-toggle="select2">
        <option value="">{{trans('Dashboard::dashboard.choose')}}</option>        
        @foreach($data->pages as $page)
        <option value="{{$page->id}}" {{$page->id == ( isset($model) ? $model->page_id : old('page_id') ) ? 'selected' : ''}} >{{ $page->display_name }}</option>
        @endforeach
    </select>
</div>

<div class="form-group row p-0 m-0 mb-5">
    <label class="col-form-label">{{ trans('Section::section.form.status') }} :</label>
