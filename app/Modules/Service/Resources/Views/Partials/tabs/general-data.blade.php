...
<div class="tab-content mt-5" id="myTabContent">
    ...
    <div class="tab-pane fade {{$key == 0 ? 'active show' : '' }}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
        <div class="form-group p-0 m-0 mb-5 pt-5">
            <label class="col-form-label">{{ trans('Service::service.form.title_'.$lang['prefix']) }} :</label>
            <input type="text" class="form-control col-md-8" name="title_{{$lang['prefix']}}" value="{{ isset($model) ? $model->{'title_'.$lang['prefix']} : old('title_'.$lang['prefix']) }}" placeholder="{{ trans('Service::service.form.title_'.$lang['prefix']) }}">
        </div>
        <div class="form-group p-0 m-0 mb-5 pt-5">
            <label class="col-form-label">{{ trans('Service::service.form.description_'.$lang['prefix']) }} :</label>
            <textarea  class="form-control col-md-8 ckeditor5" name="description_{{$lang['prefix']}}" placeholder="{{ trans('Service::service.form.description_'.$lang['prefix']) }}" cols="30" rows="10">{{ isset($model) ? $model->{'description_'.$lang['prefix']} : old('description_'.$lang['prefix']) }}</textarea>
        </div>
    </div>
    @endforeach
</div>
...
<div class="form-group  p-0 m-0 mb-5">
    <label class="">{{ trans('Service::service.form.category') }} :</label>
    <select name="category_id" class="form-control col-md-8" data-toggle="select2">
        ...
        <option value="{{$category->id}}" {{$category->id == ( isset($model) ? $model->category_id : old('category_id') ) ? 'selected' : ''}} >{{ $category->display_name }}</option>
        @endforeach
    </select>
</div>

<div class="form-group row p-0 m-0 mb-5">
    <label class="col-form-label">{{ trans('Service::service.form.sort') }} :</label>
...
<div class="form-group row p-0 m-0 mb-5">
    <label class="col-form-label">{{ trans('Service::service.form.status') }} :</label>
    <div class="col-12 p-0">
...
<div class="form-group row p-0 m-0 mb-5">
    <label class="col-form-label">{{trans('Service::service.form.image')}}</label>
    <div class="col-12 p-0">
        <div class="image-input image-input-outline" id="kt_image_5" style="background-image: url({{ isset($model) ? $model->image_url : asset(config('modules.site_configs.default_upload_img'))  }})">
            ...
            <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="{{trans('Service::service.form.change_image_p')}}">
