...
<div class="form-group  p-0 m-0 mb-5">
    ...
    <select name="parent_id" class="form-control col-md-8" data-toggle="select2">
        ...
        @foreach($data->parents as $oneCategory)
        <option value="{{$oneCategory->id}}" {{$oneCategory->id == ( isset($model) ? $model->parent_id : old('parent_id') ) ? 'selected' : ''}} >{{ $oneCategory->name }}</option>
        @endforeach
    </select>
</div>
