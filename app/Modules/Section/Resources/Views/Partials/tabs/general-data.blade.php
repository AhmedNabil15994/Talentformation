...
<div class="tab-content mt-5" id="myTabContent">
    ...
    <div class="tab-pane fade {{$key == 0 ? 'active show' : '' }}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
        ...
        <div class="form-group  p-0 m-0 mb-5 pt-5">
            ...
            <textarea  class="form-control ckeditor5 col-md-8" name="description_{{$lang['prefix']}}" placeholder="{{ trans('Section::section.form.description_'.$lang['prefix']) }}" cols="30" rows="10">{{ isset($model) ? $model->{'description_'.$lang['prefix']} : old('title_'.$lang['prefix'])}}</textarea>
        </div>
    </div>
    @endforeach
</div>
...
<div class="form-group  p-0 m-0 mb-5 pt-5">
        <label for="inputEmail3">{{ trans('Section::section.form.icon') }} :</label>                        
        <input type="text" class="form-control col-md-8" name="icon" value="{{ isset($model) ? $model->icon : old('icon')}}" placeholder="{{ trans('Section::section.form.icon') }}">
    </div>
